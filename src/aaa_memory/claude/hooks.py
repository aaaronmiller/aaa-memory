...
Uses the actual vault schema: id, turn_id, agent, session_id, turn_index,
turn_type, raw_text, created_at, metadata."""
...
def vault_path():
    """Lazy vault path — respects AAA_MEMORY_VAULT env var at call time."""
    return config.get_vault()
...
def store_turn(agent: str, prompt: str, response: str,
               session_id: str = None, project: str = None) -> str:
...
def auto_compile(agent: str, raw_text: str, session_id: str) -> None:
    ...
    try:
        ...
        for elem in elements:
            inject_metadata(elem, source_file=f"session:{session_id}",
                          classification="transcript")
        files = compile_to_wiki(elements, wiki_base=config.get_wiki())
        if files:
            import sqlite3
            conn = sqlite3.connect(str(vault_path()))
